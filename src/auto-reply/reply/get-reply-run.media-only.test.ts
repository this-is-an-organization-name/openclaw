...
describe("runPreparedReply media-only handling", () => {
  ...
  it("allows media-only prompts and omits thread context note in queued followups", async () => {
  ...
  it("does not include thread history context on follow-up turns", async () => {
  ...
  it("does not include thread starter context on follow-up turns when history is absent", async () => {
    const result = await runPreparedReply(
      baseParams({
        isNewSession: false,
        ctx: {
    ...
    expect(call?.followupRun.prompt).not.toContain("starter message");
  });

  it("omits both thread history and thread starter notes on follow-up turns", async () => {
    const result = await runPreparedReply(
      baseParams({
        isNewSession: false,
        ctx: {
