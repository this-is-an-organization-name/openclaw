...
import { isSubagentSessionKey } from "../../routing/session-key.js"; // tmpfix: sameModelRetries
import { normalizeOptionalString } from "../../shared/string-coerce.js";
import { sanitizeForLog } from "../../terminal/ansi.js";
import { sleep } from "../../utils.js"; // tmpfix: sameModelRetries
...
const MAX_SAME_MODEL_IDLE_TIMEOUT_RETRIES = 3;
      ...
      try {
        ...
        while (true) {
          ...
          const attempt = await runEmbeddedAttemptWithBackend({
            // tmpfix: retry dedup — let attempt.ts clean up duplicate user messages
          ...
          // tmpfix: sameModelRetries — subagent timeout fast-fail
          if (timedOut && !timedOutDuringCompaction && isSubagentSessionKey(params.sessionKey)) {
            log.warn(
          ...
          // tmpfix: stream degeneration loop detection — skip failover on degeneration
          const effectiveAssistantDecision: typeof assistantFailoverDecision = noFailover
            ? { action: "continue_normal" }
            : assistantFailoverDecision;

          const assistantFailoverOutcome = await handleAssistantFailover({
            initialDecision: effectiveAssistantDecision,
            aborted,
            externalAbort,
            fallbackConfigured,
            failoverFailure,
