...
import { resolvePrependContextItems } from "../../plugins/hook-before-agent-start.types.js";
import type {
...
function resolvePrependContextToString(
  val: Parameters<typeof resolvePrependContextItems>[0],
): string | undefined {
  const content = resolvePrependContextItems(val).map(i => i.content).join("\n\n");
  return content || undefined;
}

function resolveAppendContextToString(
  val: Parameters<typeof resolvePrependContextItems>[0],
): string | undefined {
  const content = resolvePrependContextItems(val).map(i => i.content).join("\n\n");
  return content || undefined;
  ...
  return {
    prompt:
      joinPresentTextSegments([
        resolvePrependContextToString(promptBuildResult?.prependContext),
        resolvePrependContextToString(legacyResult?.prependContext),
        params.prompt,
        resolveAppendContextToString(promptBuildResult?.appendContext),
        resolveAppendContextToString(legacyResult?.appendContext),
      ]) ?? params.prompt,
