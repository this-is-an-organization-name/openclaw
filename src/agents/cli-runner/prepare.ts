...
import { resolvePrependContextItems } from "../../plugins/hook-types.js";
...
export async function prepareCliRunContext(
  ...
  if (hookRunner?.hasHooks("before_prompt_build") || hookRunner?.hasHooks("before_agent_start")) {
    try {
      ...
      const prependContextText = resolvePrependContextItems(hookResult.prependContext)
        .map(item => item.content)
        .join("\n\n");
      if (prependContextText) {
        preparedPrompt = `${prependContextText}\n\n${preparedPrompt}`;
      }
      const appendContextText = resolvePrependContextItems(hookResult.appendContext)
        .map(item => item.content)
        .join("\n\n");
      if (appendContextText) {
